...
export class Path extends THREE.Group {
    ...
    constructor(parentObj, index, checkpointFloating, checkpointGrounded, FANUCorigin){
        
        console.log('NEW PATH NOW');
        
        super();

        this.eventEmitter = new EventEmitter();

        console.log('**************************************************** FANUC ORIGIN: ', FANUCorigin);
        
        this.FANUCorigin = FANUCorigin;
    ...
    updateSpline(){
        ...
        // Directional Arrow for spline
        /*var arrowGeom = new THREE.PlaneGeometry( 5, 20, 32 );
        var materialPlane = new THREE.MeshBasicMaterial( {texture: this.textureArrow, color: 0xffff00, side: THREE.DoubleSide} );
        var arrowPlane = new THREE.Mesh( arrowGeom, materialPlane );
        scene.add( arrowPlane );*/

        //const material = new MeshLineMaterial({color: 0xffffff, lineWidth: 10});
    ...
    updateHeightLinesAndFloorMarks(){
        ...
        //console.log('HEIGHTLINES: ', this.heightLines.length);
    ...
    onGroundPlaneIntersection(newRay, newPosition, mode){
        ...
        if (!checkpointSelected && mode === 0){
            ...
            console.log('NO CHECKPOINTS SELECTED. DESELECT ALL CHECKPOINTS');
    ...
    updatePathData(){
        ...
        this.pathData.checkpoints.forEach(checkpoint => {
            ...
            if (checkpoint.name === this.selectedCheckpoint.name){
                ...
                let selectedLocal = new THREE.Vector3();
                selectedLocal.copy(this.selectedCheckpoint.getWorldPosition());

                this.FANUCorigin.worldToLocal(selectedLocal);
                ...
                checkpoint.posXFANUC = -1 * selectedLocal.y;
                checkpoint.posYFANUC = selectedLocal.x;
                checkpoint.posZFANUC = selectedLocal.z;
                checkpoint.orientation = this.selectedCheckpoint.getOrientation();

                //console.log('NEW FANUC POS: ', -1 * checkpoint.posYFANUC, checkpoint.posXFANUC, checkpoint.posZFANUC);
            }
        });

        if (!exists){

            // Change coordinates for FANUC
            ...
            this.pathData.checkpoints.push({
                ...
                "posXFANUC" : -1 * selectedLocal.y,
                "posYFANUC" : selectedLocal.x,
                "posZFANUC" : selectedLocal.z,

                "orientation" : this.selectedCheckpoint.getOrientation() // orientation of the footprint
            });

            console.log('NEW CP: ', -1 * selectedLocal.y, selectedLocal.x, selectedLocal.z);
        }
    }
    ...
    checkpointsLookAt(camPosition){

        let cameraWorldPos = new THREE.Vector3();
        cameraWorldPos.copy(camPosition);
    ...
    update(deltaTime, elapsedTime, frameCount){
        ...
        if (this.checkpointTouchActive){
            ...
            console.log('touching point', this.checkpointTouchCount);

            if (this.checkpointTouchCount > 10){

                //if (this.selectedCheckpoint !== null) this.selectedCheckpoint.activateRotation(elapsedTime);
    ...
    /*
    rotateCheckpoint(delta){
        //console.log('rotateCheckpoint');
        if (this.selectedCheckpoint !== null && this.selectedCheckpoint.isRotationActive()){
            this.selectedCheckpoint.rotateFootprint(delta);
            this.updatePathData();
        }
    }
